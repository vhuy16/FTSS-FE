...
export default function ListOrderDeliveryTable() {
  ...
  useEffect(() => {
    if (status === "All" && !searchValue) {
      ...
      setOrders(
        listOrder.filter((order) => order.oderCode.toLowerCase().includes(searchValue) && order.setupPackage != null)
      );
    } else if (status !== "All" && !searchValue) {
      ...
      setOrders(
        listOrder.filter(
          (order) =>
            order.oderCode.toLowerCase().includes(searchValue) && order.status === status && order.setupPackage != null
        )
      );
    }
