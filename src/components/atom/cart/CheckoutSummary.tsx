    ...
    return (
        <CheckoutSummaryWrapper>
            ...
            <div className="order-list grid">
                {listCart?.map((cart) => {
                    return (
                        <div className="order-item grid" key={cart.item.id}>
                            <div className="order-item-img">
                                <img src={cart.item.images[0]} className="object-fit-cover" alt="" />
                            </div>
                            <div className="order-item-info flex justify-between">
                                <div className="order-item-info-l">
                                    ...
                                    <p className="text-title-2xsm font-bold text-outerspaace">
                                        Phân loại: &nbsp;
                                        <span className="text-gray font-normal">{cart.item.categoryName}</span>
                                    </p>
                                </div>
                                <div className="order-item-info-r text-gray font-bold text-base">
                                    {currencyFormat(cart.quantity * cart.item.price)}
                                </div>
                            </div>
                        </div>
                    );
